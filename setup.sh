    ...
    if [[ "$EMBED_PROVIDER" == "openai" ]]; then
        if [[ -n "${OPENAI_API_KEY:-}" ]]; then
            ...
            if [[ -z "$OPENAI_KEY" ]]; then
                warn "No API key provided. Set OPENAI_API_KEY before running the server."
            fi
        fi
    fi

    # Ollama model pull
    if [[ "$EMBED_PROVIDER" == "ollama" ]] && check_cmd ollama; then
        if confirm "Pull Ollama model '$EMBED_MODEL' now?"; then
...
wait_healthy() {
    ...
    if [[ "$EMBED_PROVIDER" == "tei" ]]; then
        info "Waiting for TEI embedding server (first run downloads ~500MB model, may take a few minutes)..."
        retries=120
        while ! curl -sf http://localhost:8090/health &>/dev/null; do
            retries=$((retries - 1))
            if [[ $retries -le 0 ]]; then
                warn "TEI server did not become healthy in time — it may still be downloading the model"
                warn "Check with: curl http://localhost:8090/health"
                warn "Or watch logs: docker compose logs -f embeddings"
                return 0
            fi
            sleep 5
        done
...
run_embedding() {
    ...
    if [[ "$EMBED_PROVIDER" == "tei" ]]; then
        if ! curl -sf http://localhost:8090/health &>/dev/null; then
            warn "TEI server not reachable — skipping embedding"
...
register_mcp() {
    ...
    if ! check_cmd claude; then
        ...
        printf "    -e DATABASE_URL=%s \\\\\n" "$db_url"
        printf "    -e EMBEDDING_URL=%s \\\\\n" "$EMBED_URL"
        printf "    -e EMBEDDING_MODEL=%s \\\\\n" "$EMBED_MODEL"
        printf "    -e EMBEDDING_DIMS=%s \\\\\n" "$EMBED_DIMS"
        printf "    -e EMBEDDING_PROVIDER=%s \\\\\n" "$EMBED_PROVIDER"
        printf "    -e WATCH_PATHS=%s \\\\\n" "$NOTES_DIR"
        printf "    -- %s\n\n" "$server_bin"
        return 0
    fi
